...
(c) 2018-2020 Michael Bell/Rockhawk (improvements of work management between threads, and some more) (https://github.com/MichaelBell/) */
...
void Miner::init(const MinerParameters &minerParameters) {
	...
	for (uint64_t i(0) ; i < _primes.size() ; i++) {
		if (i == _parameters.primorialNumber && _parameters.primorialNumber != 0)
			...
			if (_primorial*_primes[i] >= primorialLimit) {
	...
	std::cout << "Estimated additional factors: " << additionalFactorsCountEstimation << " (allocated per iteration: " << additionalFactorsEntriesPerIteration << ")" << std::endl;
	{
		...
		try {
			_modularInverses.resize(_primes.size());
		}
		catch (std::bad_alloc& ba) {
		...
		for (uint16_t j(0) ; j < _parameters.threads ; j++) {
			threads[j] = std::thread([&, j]() {
				...
				for (uint64_t i(_parameters.primorialNumber + j*blockSize) ; i < endIndex ; i++) {
					...
					_modularInverses[i] = static_cast<uint32_t>(mpz_get_ui(modularInverse.get_mpz_t()));
				}
			});
		}
		for (uint16_t j(0) ; j < _parameters.threads ; j++) threads[j].join();
		std::cout << "Tables of " << _modularInverses.size() - _parameters.primorialNumber << " modular inverses generated in " << timeSince(t0) << " s (" << (_modularInverses.size() - _parameters.primorialNumber)*sizeof(decltype(_modularInverses)::value_type) << " bytes)." << std::endl;
	}
...
void Miner::_doSieveTask(Task task) {
	...
	const uint64_t workIndex(task.workIndex), sieveIteration(task.sieve.iteration), firstPrimeIndex(_parameters.primorialNumber), tupleSize(_parameters.pattern.size());
